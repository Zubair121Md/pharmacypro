...
from sqlalchemy import create_engine, Column, Integer, String, DateTime, Boolean, Text, ForeignKey, Index, Numeric
...
DATABASE_URL = os.getenv(
    "DATABASE_URL", 
    "sqlite:///./pharmacy_revenue.db"  # Use SQLite for simplicity
)
...
def ensure_unmatched_schema():
    try:
        with engine.connect() as conn:
            ...
            cols = conn.execute("PRAGMA table_info(prms_unmatched)").fetchall()
            existing = {row[1] for row in cols}  # row[1] is column name
            # Add missing columns without dropping data
            if 'product' not in existing:
                conn.execute("ALTER TABLE prms_unmatched ADD COLUMN product VARCHAR(200)")
            if 'quantity' not in existing:
                conn.execute("ALTER TABLE prms_unmatched ADD COLUMN quantity INTEGER")
            if 'amount' not in existing:
                conn.execute("ALTER TABLE prms_unmatched ADD COLUMN amount NUMERIC(10,2)")
